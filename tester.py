...
from vae_trainer import VAE, PatchDiscriminator, gan_loss, perceptual_loss, create_dataloader
...
def test_webdataset_dataloader():
    ...
    dataloader = create_dataloader(dataset_url, batch_size=2, num_workers=4, world_size=1, rank=0)
...
def test_train_loop():
    ...
    print(f"Reconstructed image shape: {reconstructed.shape} (Expected: [1, 3, 128, 128])")
    ...
    print(f"Lambda value (weight): {lambda_val.item()} (Expected: ~1.0 depending on losses)")
